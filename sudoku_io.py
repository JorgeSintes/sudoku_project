...
def show_image(img: np.ndarray, contours=None, scale: Optional[float] = 1):
    ...
    scaled_img = cv2.resize(
        out, (int(scale*img.shape[1]), int(scale*img.shape[0])))
...
def find_sudoku(raw_img: np.ndarray, final_fig_size: Optional[int] = 300) -> np.ndarray:
    ...
    contours_areas = [(contour, cv2.contourArea(contour))
                      for contour in contours]  # Extracting contour area
    # sorting them by area
    contours_areas.sort(key=lambda contour: contour[1], reverse=True)
    ...
    # sort them by size of the sum of x and y
    pts1 = pts1[np.argsort(np.sum(pts1, axis=-1))]
    pts2 = np.float32([[0, 0], [final_fig_size, 0], [0, final_fig_size], [
                      final_fig_size, final_fig_size]])
    if pts1[1, 0] < pts1[2, 0]:  # Correct if lowerleft and upperright corners are swapped
        pts2 = np.float32([[0, 0], [0, final_fig_size], [final_fig_size, 0], [
                          final_fig_size, final_fig_size]])

    # Transform and return
    M = cv2.getPerspectiveTransform(pts1, pts2)
    final_img = cv2.warpPerspective(
        raw_img, M, (final_fig_size, final_fig_size))
...
def get_sudoku_images(sudoku_path: str, img_size: Optional[int] = 28, threshold: Optional[float] = 0.3) -> np.ndarray:
    ...
    for i in range(9):
        for j in range(9):
            _, sudoku[i, j] = cv2.threshold(cv2.cvtColor(
                img[indeces[i]:indeces[i+1], indeces[j]:indeces[j+1]], cv2.COLOR_RGB2GRAY)/255, threshold, 1, cv2.THRESH_BINARY)
